        ...
        <main>
            ...
            <section v-if="!user" class="login">
                ...
                <form v-else @submit.prevent="submitCode">
                    ...
                    <label>One-time code <input v-model.trim="code" inputmode="numeric" autocomplete="one-time-code" required /></label>
                    <button :disabled="busy" type="submit">Verify</button>
                    <button type="button" class="secondary" @click="cancelCode">Cancel</button>
                </form>
                <p class="muted">Your password is used for this sign-in only. The Android Keystore protects the session cookies.</p>
            </section>
            ...
            <template v-else>
                <section v-if="selected" class="details">
                    ...
                    <template v-if="selected.authorId === user.id">
                        ...
                        <label class="picker">Change image <input type="file" accept="image/*" @change="chooseImage" /></label>
                ...
                <section v-else>
                    <div class="profile"><img v-if="user.userIcon || user.currentAvatarImageUrl" :src="user.userIcon || user.currentAvatarImageUrl" alt="" /><div><strong>{{ user.displayName }}</strong><small>{{ user.username }}</small></div></div>
                    <nav aria-label="VRCX sections">
                        <button v-for="tab in tabs" :key="tab" :class="{ active: currentTab === tab }" @click="switchTab(tab)">{{ tab }}</button>
                    </nav>
                    ...
                    <button v-for="item in items" :key="item.id || item.objectId" class="item" @click="openItem(item)">
                        <img v-if="item.thumbnailImageUrl || item.imageUrl || item.userIcon" :src="item.thumbnailImageUrl || item.imageUrl || item.userIcon" alt="" />
                        <span><strong>{{ item.name || item.displayName || item.objectName || item.id }}</strong><small>{{ item.description || item.statusDescription || item.type }}</small></span>
...
async function loadTab() { await run(loadItems); }

async function openItem(item) {
    if (currentTab.value === 'Worlds' || currentTab.value === 'My Worlds' || currentTab.value === 'Avatars') {
        const data = await run(() => currentTab.value === 'Avatars' ? api.getAvatar(item.id) : api.getWorld(item.id));
        if (data) {
            selected.value = data;
...
async function saveDetails() {
    ...
    const data = await run(() => save(selected.value.id, { id: selected.value.id, name: editName.value, description: editDescription.value }));
...
async function saveImage() {
    ...
    if (data) { selected.value = data; imageData.value = ''; preview.value = ''; }
}
...
const onResume = () => { if (document.visibilityState === 'visible' && user.value) loadTab(); };
onMounted(() => { restore(); document.addEventListener('visibilitychange', onResume); });
onUnmounted(() => document.removeEventListener('visibilitychange', onResume));
</script>

<style scoped>
.mobile-shell { min-height: 100dvh; color: #e9edf3; background: #16191e; font: 15px system-ui, sans-serif; }
header { height: 56px; display: flex; gap: 12px; align-items: center; padding: 0 16px; background: #242930; position: sticky; top: 0; z-index: 2; }
header small, .profile small, .item small { display: block; color: #aab4c1; font-size: 11px; font-weight: normal; }
.right { margin-left: auto; }
main { max-width: 780px; margin: auto; padding: 16px; padding-bottom: 56px; }
button, input, textarea, select { font: inherit; }
button, .picker { min-height: 44px; border: 0; border-radius: 7px; padding: 10px 14px; color: inherit; background: #344b66; cursor: pointer; }
button:disabled { opacity: .5; }
input, textarea, select { min-height: 44px; width: 100%; border: 1px solid #586474; border-radius: 6px; padding: 9px; background: #242930; color: inherit; }
label { display: block; margin: 12px 0; }
.login { max-width: 440px; margin: 8vh auto; }
.login button { width: 100%; margin-top: 10px; }
.secondary { background: #39414a; }
.muted, .empty { color: #aab4c1; }
.error { background: #60313b; border-radius: 6px; padding: 12px; }
nav { display: flex; overflow-x: auto; gap: 6px; padding: 14px 0; }
nav button { flex: none; background: #30353c; white-space: nowrap; }
nav button.active { background: #42638a; }
.profile, .item, .search { display: flex; align-items: center; gap: 12px; }
.profile img { width: 54px; height: 54px; border-radius: 50%; object-fit: cover; }
.search input { min-width: 0; }
.item { width: 100%; text-align: left; background: #242930; border-radius: 0; border-bottom: 1px solid #343b44; }
.item img { width: 56px; height: 56px; object-fit: cover; border-radius: 5px; }
.item span:nth-child(2) { min-width: 0; flex: 1; overflow-wrap: anywhere; }
.item small { max-height: 3em; overflow: hidden; }
.details > img { display: block; width: 100%; max-height: 280px; object-fit: cover; }
.picker { display: block; background: #39414a; }
.picker input { border: 0; padding: 5px 0; }
.preview img { width: min(100%, 280px); display: block; margin: 14px 0; }
</style>
